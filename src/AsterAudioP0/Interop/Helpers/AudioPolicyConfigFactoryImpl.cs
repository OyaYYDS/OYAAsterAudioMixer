...
// .NET 8 经典 COM 互操作不支持 InterfaceIsIInspectable 的 RCW 强转
// （PlatformNotSupportedException），因此绕过 RCW，直接读 vtable 第 25/26/27 槽
// 生成委托调用。槽位编号经 EarTrumpet 双 Impl（19 个占位方法 + IInspectable 6 槽）
// 与 SoundSwitch 实现交叉验证。
//
// 接口 IID（两套，按系统构建分流，与 EarTrumpet 一致）：
//   21H2+ / Win11 : ab3d4648-e242-459f-b02f-541c70306324
//   旧版          : 2a59116d-6c4f-45e0-a74f-707e3fef9258
...
internal class AudioPolicyConfigFactoryImpl : IAudioPolicyConfigFactory
{
    ...
    // CLI 短生命周期，不实现 IDisposable；进程退出时 COM 指针随进程释放
}
